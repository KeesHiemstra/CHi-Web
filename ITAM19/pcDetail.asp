...
'Special report
'-------------------------------------------------------------CK---------------
If Session("UID") <> "KEES-QUICK" Then
	strSQL = "SELECT * FROM ITAMData.dbo.acReportOnRfc AS R WHERE R.[SerialNo] = '" & strSerialNo & "' and R.[DTDeletion] IS NULL "

	objRs.Open strSQL, objConn

	If Not objRs.EOF Then
		Response.Write "<tr><td colspan=4><b>Special report</td></tr>"

		While Not objRs.Eof
			Response.Write "<tr>"
      Response.Write "<tr><td>Change reference</td><td colspan=""3"">" & objRs("ChangeRef") & "</td></tr>"
      Response.Write "<tr><td>Report date</td><td colspan=""3"">" & CISODateTime(objRs("DTCreation")) & "</td></tr>"
			Response.Write "</tr>"
